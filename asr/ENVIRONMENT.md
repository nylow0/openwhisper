...
The reusable Python ASR package lives in this directory. Run NeMo/Parakeet
environment checks from here, not from the desktop app directory.
...
- `uv` was not initially on PATH; installed via `https://astral.sh/uv/install.ps1` to the user's local bin directory.
- `uv python find` resolved to `cpython-3.11.15-windows-x86_64-none` managed by `uv`.

## Blockers

None for the environment spike. The next step is a model-load test
(Parakeet batch spike) to confirm the runtime works end-to-end.
